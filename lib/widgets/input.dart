        ...
        child: items.isEmpty
            ...
            : ReorderableListView.builder(
                ...
                onReorder: (oldIndex, newIndex) {
                  ...
                  nextItems.insert(targetIndex, item);
                  onChange(nextItems);
                },
              ),
      );
...
class MapInputPage extends StatelessWidget {
  ...
  final List<Widget> Function(MapEntry<String, String> item)? trailingBuilder;
  final MapEntry<String, String> Function(MapEntry<String, String> item)?
      editItemBuilder;
  final MapEntry<String, String> Function(
  ...
  Future<void> _handleAddOrEdit([MapEntry<String, String>? item]) async {
    final editItem =
        item != null && editItemBuilder != null ? editItemBuilder!(item) : item;

    String? uniqueValidator(String? value) {
      final nextKey =
          valueBuilder?.call(item, MapEntry(value ?? "", "")).key ?? value;
      final index = items.indexWhere(
        (entry) => entry.key == nextKey,
      );
      final current = item?.key == nextKey;
      if (index != -1 && !current) {
    ...
    final keyField = Field(
      ...
      value: editItem == null ? "" : editItem.key,
      validator: uniqueValidator,
    );

    final valueField = Field(
      label: valueLabel ?? appLocalizations.value,
      value: editItem == null ? "" : editItem.value,
    );
    ...
    final nextValue = valueBuilder?.call(item, value) ?? value;
    if (item != null) {
      nextItems[index] = nextValue;
    } else {
      nextItems.add(nextValue);
    }
